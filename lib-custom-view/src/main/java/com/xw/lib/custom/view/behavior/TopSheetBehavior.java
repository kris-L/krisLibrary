...
package com.xw.lib.custom.view.behavior;
...
public class TopSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            ...
            return MathUtils.constrain(top, mHideable ? -child.getHeight() : mMaxOffset,mMinOffset);
        }
    ...
    private boolean elevationWithAnchor;
    ...
    public TopSheetBehavior(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.SheetBehavior_Layout);
        TypedValue value = a.peekValue(R.styleable.SheetBehavior_Layout_sheet_peekHeight);
        if (value != null && value.data == PEEK_HEIGHT_AUTO) {
            setPeekHeight(value.data);
        } else {
            setPeekHeight(a.getDimensionPixelSize(
                    R.styleable.SheetBehavior_Layout_sheet_peekHeight, PEEK_HEIGHT_AUTO));
        }
        setHideable(a.getBoolean(R.styleable.SheetBehavior_Layout_sheet_hideable, false));
        setSkipCollapsed(a.getBoolean(R.styleable.SheetBehavior_Layout_sheet_skipCollapsed,
                false));
        setAnchor(a.getResourceId(R.styleable.SheetBehavior_Layout_sheet_anchor,0));
        setElevationWithAnchor(a.getBoolean(R.styleable.SheetBehavior_Layout_sheet_elevationWithAnchor,true));
        a.recycle();
        ViewConfiguration configuration = ViewConfiguration.get(context);
        mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();
    }

    public void setElevationWithAnchor(boolean withAnchor) {
        this.elevationWithAnchor = withAnchor;
    }

    public void setAnchor(@IdRes int id) {
        this.anchorId = id;
    }
...
//        mMaxOffset = Math.max(mParentHeight - peekHeight, mMinOffset);
        if(anchorView != null) {
            mMinOffset = anchorView.getHeight();
            if(elevationWithAnchor){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    child.setElevation(anchorView.getElevation());
                }
            }
        }else{
            mMinOffset = 0;
        }
        mMaxOffset = -(child.getHeight() - peekHeight) + mMinOffset;
        log("mMinOffset="+mMinOffset+",mMaxOffset="+mMaxOffset);
        log("child.getHeight()="+child.getHeight()+",peekHeight="+peekHeight);
    ...
    boolean shouldHide(View child, float yvel) {
        ...
        log("mMaxOffset="+mMaxOffset+",newTop="+newTop+",mPeekHeight="+mPeekHeight);
        return Math.abs(mMaxOffset - newTop) / (float) mPeekHeight > HIDE_THRESHOLD;
    }
